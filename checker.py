...
CHROME_DRIVER_LOCATION = "/Users/kevin.notohamiprodjo/Downloads/chromedriver"
...
ALL_SITES_DICT = {
    ...
    "Wishon Point (Bass Lake)": {"url": NATIONAL_URL_TEMPLATE.format(232911), "tags": ["weekend", "bass_lake"]},
    "Cedar Bluff (Bass Lake)": {"url": NATIONAL_URL_TEMPLATE.format(232912), "tags": ["weekend", "bass_lake"]},
    "Forks Campground (Bass Lake)": {"url": NATIONAL_URL_TEMPLATE.format(232878), "tags": ["weekend","bass_lake"]},
    "Spring Cover (Bass Lake)": {"url": NATIONAL_URL_TEMPLATE.format(232801), "tags": ["weekend","bass_lake"]},
    ...
    "Calaveras Big Trees": {"url": CALIFORNIA_URL, "tags": ["weekend", "northern_cal"]},
}
...
def _get_date_list(year, start_tuple, end_tuple, weekday=None):
    ...
    if weekday:
        ...
        day += datetime.timedelta(
            days=weekdays_dict[weekday.lower()] - day.weekday())
        delta = 7
...
def check_one_date(date, tag="yosemite", park=None, output_file=None):
    ...
    while True:
        ...
        for date in sorted(available.keys()):
            value = available[date]
            msg = "{} has {} available".format(date, len(value))
            print(msg)
        _print_to_file(available, output_file)
        print("Sleeping for 20 minutes, until {}".format(datetime.datetime.now() + datetime.timedelta(minutes=20)))
        time.sleep(60 * 20)
...
def check_date_range(start_date="8-20", end_date="9-1",
    ...
    if not re.match(r"\d+-\d+", str(start_date)) and re.match(r"\d+-\d+", str(end_date)):
        raise ValueError("Expecting date to look like this: 5-11")
    dates = _get_date_list(YEAR, start_date.split("-"),
                           end_date.split("-"), weekday)
    sites_dict, tag = get_sites_dict(tag, park)
    if not output_file:
...
def check_saturdays_in_range(start_date="8-20", end_date="9-1",
    ...
    check_date_range(start_date, end_date, tag, park,
                     output_file, weekday="saturday")

def check_fridays_in_range(start_date="8-20", end_date="9-1",
                             tag="yosemite", park=None, output_file=None):
    check_date_range(start_date, end_date, tag, park,
                    output_file, weekday="friday")
...
def check_sites(dates_list, sites_dict):
    ...
    batches = [sites[i:i + NUM_PARALLEL_JOB]
               for i in range(0, len(sites), NUM_PARALLEL_JOB)]
    first = True
    start_time = time.time()
    for a_batch in batches:
        if first:
            ...
            print("Sleeping {}s between batches to slow down calls".format(SLEEP_BETWEEN_JOBS))
        ...
        for name in a_batch:
            ...
            jobs.append(proc)
            proc.start()
            # _subprocess_check_site(name, sites_dict[name], dates_list, queue_end)
        ...
        while not queue_end.empty():
            try:
                ...
                for date, a_list in a_dict.items():
        ...
        if "recreation" in url:
            ...
            for date in dates_list:
                if first:
                    ...
                    date_availability = _check_california_next_date(driver, name, date, date_availability)    
        if any([len(val) for val in date_availability.values()]):
            print("We have availability: {}".format(date_availability))
            _print_to_file(date_availability, OUTPUT_FILE.format(name, dates_list[0], dates_list[1]))
    except Exception as e:
        import traceback
        print("Encountered exception: {}".format(e))
        traceback.print_exc()
...
def _check_california_sites(driver, name, date, available_sites):
    ...
    for i in ["Camping"]:
        try:
            ...
            pass
    time.sleep(.8)
    _enter_value(driver, DATE_ARRIVAL_ID, date.replace("-", "/"))
    time.sleep(.1)
    _select_dropdown_visible_text(driver, NUMBER_OF_NIGHTS_ID, "2")
    time.sleep(1.2)
...
def _check_california_next_date(driver, name, desired_date, available_sites):
    _click_element(driver, r'//*[@class="ui-datepicker-trigger"]')
    dates = driver.find_elements_by_xpath('//*[@data-handler="selectDay"]/a')
    desired_date_dt = datetime.datetime.strptime(desired_date, r"%m-%d-%Y")
    dates = [datetime.datetime.strptime(date.get_attribute("aria-label"), r"%d %B %Y %A") for date in dates]
    counter = 0
    while not desired_date_dt in dates:
        _click_element(driver, r'//*[@class="ui-datepicker-next ui-corner-all"]')
        dates = driver.find_elements_by_xpath('//*[@data-handler="selectDay"]/a')
        dates = [datetime.datetime.strptime(date.get_attribute("aria-label"), r"%d %B %Y %A") for date in dates]
        counter += 1
        if counter > 10:
            raise Exception("Couldn't find the date {}".format(desired_date_dt))
        time.sleep(.5)
    _click_element(driver, r'//*[@data-handler="selectDay" and ./a/text()="{}"]/a'.format(desired_date_dt.day))
    return _get_available_sites_in_california_table(
        driver, name, desired_date, available_sites)
...
def _check_out_california_campsite_page(driver, name, date, available_sites):
    ...
    for row in rows:
        ...
        if children[2].get_attribute("class") == "blue_brd_box":
            ...
            if not any(adj in site_name for adj in ["ADA", "Group", "Day", "Boat", "Bike"]):
...
def _get_available_sites_in_california_table(driver, name, date, available_sites):
    ...
    if good_children:
        for i in range(len(good_children)):
            ...
            bad_children = driver.find_elements_by_class_name("btn_green_brd")
            good_children = [
                child for child in children if child not in bad_children]
            try:
                time.sleep(.3)
                ActionChains(driver).move_to_element(
                    good_children[i]).perform()
                good_children[i].click()
                _check_out_california_campsite_page(
                    driver, name, date, available_sites)
...
def _print_to_file(available_sites_dict, output_file=OUTPUT_FILE):
    ...
    with open(output_file, "w") as open_file:
        for date in sorted(available_sites_dict.keys()):
            ...
            msg = "{} has {} available".format(date, len(value))
            if len(value) > 0:
                site = output_file.split("_")[1]
                slack_msg = "<{}|{} has {} available for {}>".format(ALL_SITES_DICT[site]["url"], site, len(value), date)
                slack_url = ""
...
if __name__ == '__main__':
    fire.Fire({
        ...
        "check_fridays_in_range": check_fridays_in_range,
